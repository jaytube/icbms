...
import static com.wz.modules.common.utils.UserUtils.getCurrentUser;
...
import com.wz.modules.sys.dao.CodeDao;
...
//        CodeCache.put(Constant.CODE_CACHE,allMap);
    ...
    @Override
    public Map<String, String> queryChildsMapByMark(String mark) {
        if (StringUtils.isEmpty(mark)) {
            throw new MyException("字典标识不能为空");
        }
        List<CodeEntity> ceList = codeDao.queryChildsByMark(mark);
        Map<String, String> map = new HashMap<String, String>();
        if (null != ceList && 0 < ceList.size()) {

            for (CodeEntity ce : ceList) {
                map.put(ce.getValue(), ce.getName());
            }
            return map;
        }
        return map;
