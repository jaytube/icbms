...
public class ImageCommand extends AbstractCommand {
    ...
    public Size applyAt(CellRef cellRef, Context context) {
        ...
        if(imgBytes == null && StringUtils.isNotBlank(text)){
            Object textObj = getTransformationConfig().getExpressionEvaluator().evaluate(text,context.toMap());
            if (textObj != null) {
                if( !(textObj instanceof String) ){
                    throw new IllegalArgumentException("text value must be String type");
                }
            }
            ...
            if(row != null && row.getCell(cellRef.getCol()) != null){
                row.getCell(cellRef.getCol()).setCellValue((String)textObj);
            }
        }
        return size;
    }
